...
Gui, 2:Add, Text, x5 h20, Custom Exe:
Gui, 2:Add, Checkbox, vguicustomLogout x90 y290 h20 Checked%customLogout%,
...
Gui, 2:Add, Edit, w150 h20 vguicustomLogoutName , %customLogoutName%
...
Gui, 8:+ToolWindow
Gui, 8:Color, %colorBG%
Gui, 8:Show, x500 y0 w519 h71 NA
Gui, 8:Add, Progress, x0 y0  w525 h15 c%cooldownColor1% BackGround%colorBG% vProg1
Gui, 8:Add, Progress, x0 y15 w525 h15 c%cooldownColor2% BackGround%colorBG% vProg2
Gui, 8:Add, Progress, x0 y30 w525 h15 c%cooldownColor3% BackGround%colorBG% vProg3
Gui, 8:Add, Progress, x0 y45 w525 h5 c%cooldownColor4% BackGround%colorBG% vProg4
Gui, 8:Add, Progress, x0 y50 w525 h5 c%cooldownColor5% BackGround%colorBG% vProg5
Gui, 8:Add, Progress, x0 y55 w525 h5 c%cooldownColor6% BackGround%colorBG% vProg6
Gui, 8:Add, Progress, x0 y60 w525 h5 c%cooldownColor7% BackGround%colorBG% vProg7
...
submit(){  
	...
	IniWrite, %guicustomLogout%, settings.ini, variables, CustomLogout
	IniWrite, %guicustomLogoutName%, settings.ini, variables, CustomLogoutName
...
readFromFile(){
	...
	IniRead, customLogout, settings.ini, variables, CustomLogout
	IniRead, customLogoutName, settings.ini, variables, CustomLogoutName
	...
	if beta = ERROR
		beta = 1
	if customLogout = ERROR
		customLogout = 0
	if customLogoutName = ERROR
		customLogoutName := "Game.exe"
	if scroll = ERROR
		scroll = 1
	if waxOffset = ERROR
		waxOffset = 0
	...
	if ( customLogout ) {
		cportsCommand := "cports.exe /close * * * * " . customLogoutName
		executable := customLogoutName
	}
